...
import { PostFinanceCheckout as Whitelabel } from "../index";
import {
    ApiConfig,
...
const transactionIframeService = new TransactionIframeService(ApiConfig);
...
describe("TransactionIframeService", () => {
    describe("javascriptUrl()", () => {
        it("should create URL for use in IFrame checkout flow javascript", (done) => {
            transactionService.create(ApiConfig.space_id, getTransactionCreate())
                .then((response: { response: http.IncomingMessage, body: Transaction }) => {
                    ...
                    return transactionIframeService.javascriptUrl(ApiConfig.space_id, transactionId);
                })
